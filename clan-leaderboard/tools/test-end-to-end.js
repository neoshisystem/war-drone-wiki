...
try {
  ...
  for (const file of ['players.json','snapshots.json','player-observations.json','player-observations-history.json','player-observations-history-s05.json','player-observations-history-s06.json','memberships.json','index.json','leagues.json']) copy(file, path.join(repoData, file), path.join(tempData, file));
  ...
  console.log('END-TO-END TEST PASS: staged synthetic S98 JSON -> canonical -> generated report, league-start metadata accepted, performance dependency present, production data and S06 history shard unchanged.');
