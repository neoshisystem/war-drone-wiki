...
for (const id of ['S01', 'S02', 'S03', 'S04', 'S05', 'S06']) if (!results[id]) fail(`missing ${id}`);
...
for (const id of ['S02', 'S03', 'S04', 'S05', 'S06']) {
...
syntheticSnapshots.snapshots.push({ snapshot_id:'S07', captured_at_utc:'2026-09-17T00:00:00Z', date_persian:'26 شهریور 1405', time_iran:'03:30', type:'delta-report', members:47, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S08', captured_at_utc:'2026-09-17T01:00:00Z', date_persian:'26 شهریور 1405', time_iran:'04:00', type:'delta-report', members:47, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S09', captured_at_utc:'2026-09-17T02:00:00Z', date_persian:'26 شهریور 1405', time_iran:'05:30', type:'delta-report', members:46, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S10', captured_at_utc:'2026-09-17T03:00:00Z', date_persian:'26 شهریور 1405', time_iran:'06:30', type:'delta-report', members:47, capacity:50 });
const s05Rows = rows('S05');
const s07Rows = s05Rows.map(row => ({ ...row }));
const s08Rows = s05Rows.map((row, index) => index === 0 ? ({ ...row, clan_medals: row.clan_medals + 100, total_kills: row.total_kills + 7 }) : ({ ...row }));
const gapPlayerId = s05Rows[0].player_id;
const s09Rows = s08Rows.filter(row => row.player_id !== gapPlayerId);
const s10Rows = s09Rows.concat([{ ...s08Rows[0], clan_medals: s08Rows[0].clan_medals + 500, total_kills: s08Rows[0].total_kills + 50 }]);
const syntheticCurrent = { snapshots: { S07: s07Rows, S08: s08Rows, S09: s09Rows, S10: s10Rows } };
const synthetic = performance.computeAll(syntheticSnapshots, [...sets, syntheticCurrent], leagues);
if (synthetic.S07.league_week !== '2026-09-17') fail('league reset date was not recognized');
if (synthetic.S07.weekly_clan_medals_earned !== 0 || synthetic.S07.weekly_kills_earned !== 0) fail('weekly metrics did not reset at league boundary');
if (synthetic.S08.weekly_clan_medals_earned !== 100 || synthetic.S08.weekly_kills_earned !== 7) fail('new-week accumulation did not restart from zero');
if (synthetic.S08.period_players?.[s05Rows[0].player_id]?.clan_medals !== 100) fail('new-week period clan delta mismatch');
if (synthetic.S08.period_players?.[s05Rows[0].player_id]?.kills !== 7) fail('new-week period kill delta mismatch');
if (synthetic.S10.period_players?.[gapPlayerId]?.clan_medals !== 0 || synthetic.S10.period_players?.[gapPlayerId]?.kills !== 0 || synthetic.S10.period_players?.[gapPlayerId]?.baseline !== true) fail('re-added player must restart as a baseline after an observation gap');
if (synthetic.S10.cumulative_players?.[gapPlayerId]?.clan_medals !== synthetic.S08.cumulative_players?.[gapPlayerId]?.clan_medals) fail('cumulative clan value must not bridge a membership gap');
if (synthetic.S10.cumulative_players?.[gapPlayerId]?.kills !== synthetic.S08.cumulative_players?.[gapPlayerId]?.kills) fail('cumulative kill value must not bridge a membership gap');
console.log('PERFORMANCE TEST PASS: period deltas, S01 baseline semantics, membership-aware weekly accumulation, league reset, cumulative player metrics, and membership-gap resets are valid.');
