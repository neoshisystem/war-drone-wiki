...
syntheticSnapshots.snapshots.push({ snapshot_id:'S08', captured_at_utc:'2026-09-17T00:00:00Z', date_persian:'26 شهریور 1405', time_iran:'03:30', type:'delta-report', members:47, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S09', captured_at_utc:'2026-09-17T01:00:00Z', date_persian:'26 شهریور 1405', time_iran:'04:00', type:'delta-report', members:47, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S10', captured_at_utc:'2026-09-17T02:00:00Z', date_persian:'26 شهریور 1405', time_iran:'05:30', type:'delta-report', members:46, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S11', captured_at_utc:'2026-09-17T03:00:00Z', date_persian:'26 شهریور 1405', time_iran:'06:30', type:'delta-report', members:47, capacity:50 });
const s07Rows = rows('S07');
const s08Rows = s07Rows.map(row => ({ ...row }));
const s09Rows = s07Rows.map((row, index) => index === 0 ? ({ ...row, clan_medals: row.clan_medals + 100, total_kills: row.total_kills + 7 }) : ({ ...row }));
...
if (synthetic.S08.weekly_clan_medals_earned !== 0 || synthetic.S08.weekly_kills_earned !== 0) fail('weekly metrics did not reset at league boundary');
if (synthetic.S09.weekly_clan_medals_earned !== 100 || synthetic.S09.weekly_kills_earned !== 7) fail('new-week accumulation did not restart from zero');
if (synthetic.S09.period_players?.[s07Rows[0].player_id]?.clan_medals !== 100) fail('new-week period clan delta mismatch');
if (synthetic.S09.period_players?.[s07Rows[0].player_id]?.kills !== 7) fail('new-week period kill delta mismatch');
if (synthetic.S11.period_players?.[gapPlayerId]?.clan_medals !== 0 || synthetic.S11.period_players?.[gapPlayerId]?.kills !== 0 || synthetic.S11.period_players?.[gapPlayerId]?.baseline !== true) fail('re-added player must restart as a baseline after an observation gap');
if (synthetic.S11.cumulative_players?.[gapPlayerId]?.clan_medals !== synthetic.S09.cumulative_players?.[gapPlayerId]?.clan_medals) fail('cumulative clan value must not bridge a membership gap');
if (synthetic.S11.cumulative_players?.[gapPlayerId]?.kills !== synthetic.S09.cumulative_players?.[gapPlayerId]?.kills) fail('cumulative kill value must not bridge a membership gap');
console.log('PERFORMANCE TEST PASS: period deltas, S01 baseline semantics, S07 membership-aware aggregation, league reset, cumulative player metrics, and membership-gap resets are valid.');
