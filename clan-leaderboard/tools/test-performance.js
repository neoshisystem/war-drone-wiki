...
syntheticSnapshots.snapshots.push({ snapshot_id:'S08', captured_at_utc:'2026-09-17T02:00:00Z', date_persian:'26 شهریور 1405', time_iran:'05:30', type:'delta-report', members:46, capacity:50 });
syntheticSnapshots.snapshots.push({ snapshot_id:'S09', captured_at_utc:'2026-09-17T03:00:00Z', date_persian:'26 شهریور 1405', time_iran:'06:30', type:'delta-report', members:47, capacity:50 });
...
const s08Rows = s07Rows.filter(row => row.player_id !== gapPlayerId);
const s09Rows = s08Rows.concat([{ ...s07Rows[0], clan_medals: s07Rows[0].clan_medals + 500, total_kills: s07Rows[0].total_kills + 50 }]);
const syntheticCurrent = { snapshots: { S06: s06Rows, S07: s07Rows, S08: s08Rows, S09: s09Rows } };
...
if (synthetic.S09.period_players?.[gapPlayerId]?.clan_medals !== 0 || synthetic.S09.period_players?.[gapPlayerId]?.kills !== 0 || synthetic.S09.period_players?.[gapPlayerId]?.baseline !== true) fail('re-added player must restart as a baseline after an observation gap');
if (synthetic.S09.cumulative_players?.[gapPlayerId]?.clan_medals !== synthetic.S07.cumulative_players?.[gapPlayerId]?.clan_medals) fail('cumulative clan value must not bridge a membership gap');
if (synthetic.S09.cumulative_players?.[gapPlayerId]?.kills !== synthetic.S07.cumulative_players?.[gapPlayerId]?.kills) fail('cumulative kill value must not bridge a membership gap');
