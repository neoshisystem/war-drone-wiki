...
try{
 ...
 if(cards!==9)throw new Error(`expected 9 archive entries, found ${cards}`);
 ...
 if(!html.includes('دوره 9'))throw new Error('missing newest period label');
 ...
 const aggregateCount=(html.match(/class="report-aggregate"/g)||[]).length;if(aggregateCount!==9)throw new Error(`expected session aggregate card on all 9 archive entries, found ${aggregateCount}`);
 ...
 if(aggregates.length!==9)throw new Error(`expected 9 aggregate values, found ${aggregates.length}`);
