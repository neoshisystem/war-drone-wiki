...
try{
 ...
 if(cards!==5)throw new Error(`expected 5 archive entries, found ${cards}`);
 for(const id of ['2026-09-14-2300','2026-09-13-2300','2026-09-13-1130','2026-09-12-1900'])if(!html.includes(`reports/${id}.html`))throw new Error(`missing archive link for ${id}`);
 if(!html.includes('clan-leaderboard.html'))throw new Error('missing S02 archive link');if(!html.includes('دوره 5'))throw new Error('missing newest period label');
 const aggregateCount=(html.match(/جمع این دوره/g)||[]).length;if(aggregateCount!==5)throw new Error(`expected session aggregate on all 5 archive entries, found ${aggregateCount}`);
