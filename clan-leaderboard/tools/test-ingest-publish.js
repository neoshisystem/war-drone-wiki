...
try {
  ...
  const report = path.join(tempReports, '2026-09-19-0045.html'); if (!fs.existsSync(report)) throw new Error('PUBLISH E2E: report was not generated');
  ...
  const index = JSON.parse(fs.readFileSync(path.join(tempData, 'index.json'), 'utf8')); if (index.reports[0].id !== '2026-09-18-0730') throw new Error('PUBLISH E2E: new report was not prepended to index');
  const snapshots = JSON.parse(fs.readFileSync(path.join(tempData, 'snapshots.json'), 'utf8')); const published = snapshots.snapshots.find((item) => item.snapshot_id === 'S98'); if (!published || published.source_report !== '/clan-leaderboard/reports/2026-09-18-0730.html') throw new Error('PUBLISH E2E: source_report not persisted'); if (published.league_boundary !== 'start' || published.boundary_label !== 'شروع لیگ جدید') throw new Error('PUBLISH E2E: league-start boundary metadata not persisted');
  const archive = fs.readFileSync(path.join(tempRoot, 'archive.html'), 'utf8'); if (!archive.includes('reports/2026-09-18-0730.html')) throw new Error('PUBLISH E2E: archive was not updated'); if (parsed.archive !== 'updated') throw new Error('PUBLISH E2E: publication result did not report archive update');
