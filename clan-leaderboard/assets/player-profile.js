(() => {
  ...
  Promise.all([
    ...
    const rankDisplay = o => {
      ...
      const movement = Number(o.rank_movement || 0);
      if (!movement) return fmt(o.rank);
      return `${fmt(o.rank)} (${movement > 0 ? '↑' : '↓'} ${fmt(Math.abs(movement))})`;
    };
    ...
    const weaponDisplay = o => {
      ...
      const part = key => {
        ...
        return d[key] == null ? fmt(value) : `${fmt(value)} (+${fmt(d[key])})`;
      };
      return `${part('25mm')} / ${part('hydra')} / ${part('hellfire')}`;
    };

    const snapshotLabel = key => {
      const meta = snapshotMeta[key];
      if (meta) return `<span class="snapshot-badge">${esc(key)}</span><span class="snapshot-date">${esc(meta.date)} · ${esc(meta.time)}</span>`;
      return `<span class="snapshot-badge">${esc(key)}</span>`;
    };

    const leaderboardColumns = [
      'رتبه', 'نام کاربری', 'سمت', 'استیج', 'مدال لیگ جاری', 'تغییر مدال لیگ',
      'مدال کل کلن', 'مدال افتخار (طلا / نقره / برنز)', 'مجموع کیل 💀', 'افزایش کیل 💀',
      'لول سلاح‌ها (توپ / هیدرا / هل‌فایر)', 'آخرین آنلاین'
    ];

    const renderObservation = (key, o) => {
      if (!o) return `<tr class="snapshot-missing"><td colspan="13">این بازیکن در Snapshot ${esc(key)} حضور نداشته است.</td></tr>`;
      return `<tr>
        ...
        <td>${esc(stageDisplay(o))}</td>
        <td>${fmt(o.league_medals)}</td>
        <td>${esc(deltaDisplay(o, 'league_medals_delta'))}</td>
        <td>${fmt(o.clan_medals)}</td>
        <td>${esc(honorDisplay(o))}</td>
    ...
    const tableRows = rows.map(({ key, obs }) => renderObservation(key, obs)).join('');
    ...
    root.innerHTML = `<div class="shell profile-shell">
      ...
      <section class="panel"><div class="profile-stats">
        ...
        <div class="profile-stat"><span>مدال افتخار</span><strong>${honorDisplay(active)}</strong></div>
        <div class="profile-stat"><span>آخرین آنلاین</span><strong>${esc(active?.last_online_display || '—')}</strong></div>
      </div></section>

      <section class="panel progression-panel"><div class="snapshot-heading"><div><span class="badge">HISTORY</span><h2>تاریخچه عملکرد کاربر</h2><p class="muted">هر سطر یک Snapshot است و ترتیب و عنوان ستون‌ها با جدول اصلی Leaderboard یکسان نگه داشته شده است. مشخصات Snapshot در ستون انتهایی هر سطر قرار دارد.</p></div></div>
        <div class="table-wrap profile-history-wrap"><table class="profile-history-table"><thead><tr>${leaderboardColumns.map(c => `<th>${c}</th>`).join('')}<th>Snapshot</th></tr></thead><tbody>${tableRows}</tbody></table></div>
      </section>

      ${!active ? '<section class="panel empty">برای این بازیکن در هیچ Snapshot ثبت‌شده‌ای رکوردی پیدا نشد.</section>' : ''}
      <section class="panel source-note"><strong>منبع داده:</strong> Snapshotهای Canonical تأییدشده. هر مقدار از همان Snapshot خوانده می‌شود و برای مقایسهٔ دقیق، مقدار جدیدی خارج از دادهٔ ثبت‌شده تولید نمی‌شود.</section>
    </div>`;
  }).catch(() => {
    root.innerHTML = '<div class="shell profile-shell"><div class="panel empty">داده پروفایل قابل بارگذاری نیست.</div></div>';
  });
})();
