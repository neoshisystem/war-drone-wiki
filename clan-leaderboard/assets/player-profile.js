(() => {
  ...
  Promise.all([
    ...
    const weeklyPlayer = performanceBySnapshot[activeRow.key]?.players?.[id] || { clan_medals: 0, kills: 0 };
    ...
    const deltaDisplay = (obs, field) => !obs ? '—' : obs.is_new ? 'جدید' : obs[field] == null ? '—' : signed(obs[field]);
