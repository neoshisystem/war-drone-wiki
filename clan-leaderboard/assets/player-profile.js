(() => {
  ...
  Promise.all([
    ...
    const labels = {
      S01: 'S01 · ۲۱ شهریور ۱۴۰۵ · ۱۹:۰۰',
      S02: 'S02 · ۲۱ شهریور ۱۴۰۵ · ۲۳:۳۰',
      S03: 'S03 · ۲۲ شهریور ۱۴۰۵ · ۱۱:۳۰'
    };
    ...
    const stageDisplay = o => {
      if (o?.stage == null) return '—';
      return fmt(o.stage);
    };

    const deltaDisplay = (o, field) => {
      if (!o) return '—';
      if (o.is_new) return 'جدید';
      return o[field] == null ? '—' : signed(o[field]);
    };

    const honorDisplay = o => o?.honor_medals
      ? `${fmt(o.honor_medals.gold)} / ${fmt(o.honor_medals.silver)} / ${fmt(o.honor_medals.bronze)}`
      : '—';
    ...
    const snapshotLabel = key => {
      ...
      if (meta) {
        return `<span class="snapshot-badge">${esc(key)}</span><span class="snapshot-date">${esc(meta.date)} · ${esc(meta.time)}</span>`;
      }
    ...
    root.innerHTML = `<div class="shell profile-shell">
      <section class="profile-head">
        <div>
          <span class="badge">PERSIA · PLAYER</span>
          <h1>${esc(player.display_name)}</h1>
          <div class="identity">${esc(player.player_id)} · ${esc(player.role || 'Member')} · ${player.status === 'former' ? 'سابق' : 'فعال'}</div>
        </div>
        <div class="profile-actions">
          <a class="btn" href="players.html">← اعضای کلن</a>
          <a class="btn" href="index.html">لیدربورد</a>
          <a class="btn" href="member-history.html?id=${encodeURIComponent(player.player_id)}">تاریخچه عضویت</a>
        </div>
      </section>
