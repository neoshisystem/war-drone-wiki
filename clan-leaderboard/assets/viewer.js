(() => {
  ...
  fetch(source).then(r => { if(!r.ok) throw new Error('source'); return r.text(); }).then(html => {
    ...
    const table = list => `<div class="table-wrap"><table><thead><tr><th>رتبه</th><th>نام کاربری</th><th>سمت</th>${keys.map(k=>`<th>${esc(k)}</th>`).join('')}</tr></thead><tbody>${list.map(m=>`<tr><td>${esc(m.rank)}</td><td>${esc(m.name)}</td><td>${esc(m.role)}</td>${keys.map(k=>`<td>${esc(m.stats[k]||'—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    ...
    let current=mode;
    const render=()=>{const q=input.value.trim().toLowerCase();const list=members.filter(m=>([m.rank,m.name,m.role,m.movement,...Object.values(m.stats)].join(' ').toLowerCase()).includes(q));results.innerHTML=`<div class="count">${list.length} نتیجه</div>`+(current==='simple'?table(list):`<div class="members">${list.map(cardView).join('')}</div>`);sb.classList.toggle('active',current==='simple');gb.classList.toggle('active',current==='graphic');};
