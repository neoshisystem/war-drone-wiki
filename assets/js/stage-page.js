(()=>{
  ...
  if(to){to.value=(qTo>=1&&qTo<=100)?qTo:saved;to.dispatchEvent(new Event('input',{bubbles:true}));to.dispatchEvent(new Event('change',{bubbles:true}));}
  ...
  if(use&&to)use.addEventListener('click',()=>{to.value=Math.max(1,Math.min(100,Number(localStorage.getItem('wd-current-stage')||saved)));to.dispatchEvent(new Event('change',{bubbles:true}));});
  if(goal){
