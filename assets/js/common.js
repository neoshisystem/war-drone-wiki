(()=>{
  ...
  if(!document.querySelector('link[href="assets/css/mobile-plus.css"]')){const l=document.createElement('link');l.rel='stylesheet';l.href='assets/css/mobile-plus.css';document.head.appendChild(l);}
  ...
  document.querySelectorAll('[data-theme-toggle]').forEach(btn=>{const sync=()=>btn.textContent=root.dataset.theme==='dark'?'☀️':'🌙';btn.addEventListener('click',()=>{root.dataset.theme=root.dataset.theme==='dark'?'light':'dark';localStorage.setItem('wd-theme',root.dataset.theme);sync();});sync();});
  const nav=document.querySelector('.main-nav');
  if(nav){const links=[['index.html','خانه'],['academy.html','آموزش بازی'],['stages.html','تحلیل مرحله‌ها'],['clan.html','کلن و فرنزی'],['arsenal.html','سلاح‌ها و کارت‌ها'],['glossary.html','واژه‌نامه'],['advisor.html','راهنمای پیشروی']];links.forEach(([href,label])=>{let a=nav.querySelector(`a[href="${href}"]`);if(!a){a=document.createElement('a');a.href=href;nav.appendChild(a);}a.textContent=label;});}
  ...
  const replacements=[
    ...
    [/Progression/g,'پیشروی'],[/Progress/g,'پیشروی'],[/Upgrade/g,'ارتقا'],[/Weapons/g,'سلاح‌ها'],[/Weapon/g,'سلاح'],[/Run/g,'دور بازی'],[/Stage /g,'مرحله '],
    [/فارم کش/g,'فارم پول'],[/Clan Medal/g,'مدال کلن']
  ];
  function polish(rootNode){const walker=document.createTreeWalker(rootNode,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(n=>{if(!n.nodeValue||!n.nodeValue.trim())return;if(n.parentElement?.closest('script,style,code,pre'))return;let v=n.nodeValue;replacements.forEach(([r,to])=>v=v.replace(r,to));if(v!==n.nodeValue)n.nodeValue=v;});}
  polish(document.body);const mo=new MutationObserver(muts=>muts.forEach(m=>m.addedNodes.forEach(n=>{if(n.nodeType===1||n.nodeType===3)polish(n.nodeType===3?n.parentNode:n);})));mo.observe(document.body,{childList:true,subtree:true});
})();