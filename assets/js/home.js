(()=>{
  ...
  function weaponText(d){return d||{gun:'داده کافی نیست',hydra:'داده کافی نیست',hellfire:'داده کافی نیست',confidence:'راهنمای کیفی'};}
  function render(stage){
    stage=I.clampStage(stage);const p=I.progression(stage);const w=weaponText(p.currentTarget||p.nextTarget);
    ...
    if(!box)return;
    const cash=I.recommend('cash',stage,1)[0];const frenzy=stage>=8?I.recommend('frenzy',stage,4):[];const stop=I.recommend('farm99',stage,1)[0];const items=[];
    ...
    box.innerHTML=items.length?items.join(''):'<div class="recommendation"><b>فعلاً روی Progress تمرکز کن</b><span>Frenzy از حدود Stage 8 به بعد برایت وارد برنامه می‌شود. بعد از آن حتی Stageهای قدیمی‌تر مثل 5 را هم می‌توانی برای Medal تست کنی.</span></div>';
    const picks=$('homeStagePicksTitle');if(picks)picks.textContent=`گزینه‌های قابل تست تا Stage ${stage}`;
  }
