(()=>{
  ...
  function card(x,icon,goal){return `<a class="recommendation home-rec" href="stages.html?to=${x.stage}&goal=${goal}"><b>${icon} Stage ${x.stage} · ${x.name}</b><span>${x.note}</span></a>`;}
  function render(stage){
    ...
    const items=[];
    if(cash)items.push(card(cash,'💰','cash'));
    if(frenzy && (!cash||frenzy.stage!==cash.stage))items.push(card(frenzy,'🔥','frenzy'));
    if(stop && !items.some(i=>i.includes(`Stage ${stop.stage} ·`)))items.push(card(stop,'🎯','farm99'));
